...
export function ModelApplicationVideo() {
  ...
  // Autoplay video as soon as the section is scrolled into view
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Section entered view -> autoplay video
            const playPromise = video.play();
            if (playPromise !== undefined) {
              playPromise
                .then(() => setIsPlaying(true))
                .catch(() => {
                  // Autoplay policy prevented or user hasn't interacted yet
                });
            }
          } else {
            // Section scrolled out of view -> pause video to save resources
            video.pause();
            setIsPlaying(false);
          }
        });
      ...
      {
        threshold: 0.25, // Trigger as soon as 25% of section enters viewport
      }
    );
  ...
  return (
    ...
    >
      ...
      <div
        onClick={togglePlay}
        className="group relative w-full h-[360px] sm:h-[420px] lg:h-[460px] overflow-hidden bg-black cursor-pointer select-none"
        role="region"
        aria-label="The Velyra Application Ritual Video Demonstration"
      >
        {/* Autoplaying HTML5 Video with Poster Fallback */}
        <video
          ref={videoRef}
          src="/Hero.mp4"
          poster="/models/model-apply.jpg"
          muted={isMuted}
          playsInline
          loop
          preload="auto"
          className="absolute inset-0 w-full h-full object-cover object-[center_32%] transform transition-transform duration-700 ease-out group-hover:scale-[1.01]"
          aria-label="Velyra Application Ritual Dermal Demonstration"
        ...
        >
          <button
            ...
            type="button"
            onClick={togglePlay}
            className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-white/20 backdrop-blur-md border border-white/40 text-white flex items-center justify-center hover:scale-110 hover:bg-white/30 hover:border-white/60 transition-all duration-300 shadow-2xl pointer-events-auto focus:outline-none focus:ring-2 focus:ring-white/80"
        ...
        <div className="absolute inset-0 p-5 sm:p-8 lg:p-12 flex flex-col justify-between pointer-events-none z-10">
