...
export function ProductGallery({ images, productName }: ProductGalleryProps) {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row gap-4">
      ...
      {/* Main Showcase Image */}
      <div className="relative flex-1 aspect-[4/5] bg-surface-muted rounded-sm overflow-hidden border border-border-subtle group">
        <div
          className={`relative w-full h-full p-8 transition-transform duration-500 ease-out cursor-zoom-in ${
        ...
        <button
          onClick={() => setIsZoomed(!isZoomed)}
          className="absolute bottom-4 right-4 bg-surface-elevated/90 backdrop-blur-sm p-2 rounded-full text-brand-charcoal hover:bg-surface-elevated shadow-sm transition-all"
        ...
        {validImages.length > 1 && (
          <div className="lg:hidden absolute inset-y-0 inset-x-2 flex items-center justify-between pointer-events-none">
            <button
              onClick={handlePrev}
              className="pointer-events-auto p-2 bg-surface-elevated/80 backdrop-blur-sm rounded-full text-brand-charcoal shadow-sm"
